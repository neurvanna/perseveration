classdef agent_optimistic_q
    ...
    methods
        ...
        %% Constructor
        function self = agent_optimistic_q(params)
            if ~exist('params', 'var')
        ...
        function params = get_params(self)
           params = [self.alpha_plus, self.alpha_minus, self.beta, self.bias]; 
        end
        ...
        function self = set_params(self, params)
            ...
            self.alpha_plus = params(1);
            self.alpha_minus = params(2);
            self.beta = params(3);
            self.bias = params(4);
        ...
        function actionProbs = get_actionProbs(self)
            Qeff = [self.beta*self.Q(1) + self.bias, ...
                    self.beta*self.Q(2) - self.bias];
        ...
        function self = update(self, trial)
            ...
            if trial.reward
                alpha = self.alpha_plus;
            elseif ~trial.reward
                alpha = self.alpha_minus;
            end
            ...
            % Update Q
            new_Q = [NaN, NaN];
            new_Q(chosen) =   (1-alpha)   * self.Q(chosen) + alpha * trial.reward;
